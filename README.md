...
Each channel is covered in a Kodak Wratten filter, which allows only for a limited range of wavelengths to be detected into the sensor.
...
By weighting and combining the contribution of these three channels we can produce a single-colored pixel.
...
The firmware maps out each photodiode to individual ADC pins which receive the incoming voltages and process those into digital numbers on a 16-bit scale. A button function is defined to capture when the voltage from that pin changes states.
The firmware can have dark floor values set manually, as well gains per channel if needed.
Dark floors are established as the readings captured during "near absolute dark conditions" which account for sensor noise. To normalize the system, each channel's raw contribution gets the dark floor reading subtracted from it to yield a usable range. Then, values are clamped at the maximum of 65535 per diode. The firmware then prints the raw values as well as the corrected values, which can be plotted in Thonny.
...
Next steps include: mapping channel response across known wavelengths, the initial plotting script for which has also been included in this repository.
